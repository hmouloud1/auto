...
public class AccessLib{
		...
		if (type.equalsIgnoreCase("manual submission"))
		{
			...
			}else
			{
				...
				updateHtmlReport("Manual submission", 
						"User is able to perform manual submission", 
						"Bar Code:  <span class = 'boldy'>"+barCode+"</span>", "Step", "pass", "manual submission");
			}
		}
		else
		{
	...
	*/
	public static boolean createApoApplication(LinkedHashMap<String, String> row) throws Exception
	{
		...
		clickElementJs(guiMap.get("checkParticipant"));
		holdSeconds(1);
		enterTextTextArea(guiMap.get("txtParticipant"), row.get("Text_Participant"));
		...
		for(;i<segmentOption.length;i++)
		{
			...
			else
			{
				...
				else
				{
					create=false;
					updateHtmlReport("Submitting APO with case segment = ["+opt+"]",
							"Either barcode or entry of appearance error message should be displayed", 
		...
		for (HashMap.Entry<String, String> entry : row.entrySet()) 
		{
			...
			switch (entry.getKey())
			{
				...
				case "Submit": case "Reset": case "Cancel":
				{
					...
					{
						scrollToElement(replaceGui(guiMap.get("fieldHelpLink_7"),fieldName));
						highlightElement(replaceGui(guiMap.get("fieldHelpLink_7"),fieldName), "blue");
						clickElementJs(replaceGui(guiMap.get("fieldHelpLink_7"),fieldName));
					}
					else
					{
	...
	*/
	 ...
	 * @exception Exception
	*/
	public static boolean ValidateEfileRegisterRequiredFields(LinkedHashMap<String, String> row) throws Exception
	{
		boolean validate = true;
		int currentWait = setBrowserTimeOut(3);
		if(elementExists(guiMap.get("linkLogout")))
		...
		for (HashMap.Entry<String, String> entry : row.entrySet()) 
		{
					...
					)
			{
				...
				//scrollToElement(guiMap.get(replaceGui(guiMap.get("EfileFields"), fieldName)));
		...
		{
			updateHtmlReport("Validate E-File required fields", "All the fields controllers should be working as expected", 
					"Not as expected", "VP", "fail", "Validate E-File required fields");
	 ...
	 * @exception Exception
	*/
	public static boolean ValidateUserRegisterRequiredFields(LinkedHashMap<String, String> row) throws Exception
	{
		boolean validate = true;
		int currentWait = setBrowserTimeOut(3);
		if(elementExists(guiMap.get("linkLogout")))
		...
		if(validate)
		{
			updateHtmlReport("Validate User Register fields", "All the fields controllers should be working as expected", 
					"As expected", "VP", "pass", "Validate E-File required fields");
				...
				{ 
							...
							"Segment Specific Information"))))
					{
						...
						updateHtmlReport("Checking Segment Specific Information",
								"This field supposed to have values to pick up from, and should be marked as required",
								"Not as expected", "VP", "fail", "");
			...
			ArrayList<LinkedHashMap<String, String>> list) throws Exception
	{
		...
		for (LinkedHashMap<String, String> row: list) 
		{
			...
			switch (fieldName)
			{
				...
				{
					enterTextTextArea(guiMap.get("FiledOnBehalfOf"), givenValue);
					clickElementJs(guiMap.get("submitButton"));	
					holdSeconds(1);
				...
				case "Title":
				{
					scrollToElement(replaceGui(guiMap.get("fieldHelpLink_1"), "Title"));
					if(!givenValue.equals(""))
					{
	...
	*/
	public static boolean resubmitEoa(LinkedHashMap<String, String> row) throws Exception
	{
		...
		scrollToNiemElement(guiMap.get("EoaCaseNumberLinkAll_2"), j+1);
		highlightNiemElement(guiMap.get("EoaCaseNumberLinkAll_2"), "blue", j+1);
		...
		clickNiemElementJs(guiMap.get("EoaCaseNumberLinkAll_2"), j+1);
		selectElementByValue(guiMap.get("SelectIsparticipantrepresentedcounsel"), "Yes");
		holdSeconds(2);
		if(!checkElementIsEnabled(guiMap.get("submitButton")))
		{
	...
	*/
	public static boolean resubmitApo(LinkedHashMap<String, String> row) throws Exception
	{
		...
		scrollToNiemElement(guiMap.get("ApoCaseNumberLinkAll_2"), j+1);
		highlightNiemElement(guiMap.get("ApoCaseNumberLinkAll_2"), "blue", j+1);
		updateHtmlReport("resubmit existing case number", "Case number should be available",
				"As expected", "Step", "pass", "find case nuber for resubmit");
		clickNiemElementJs(guiMap.get("ApoCaseNumberLinkAll_2"), j+1);
		holdSeconds(2);
		if(!checkElementIsEnabled(guiMap.get("submitButton")))
		{
