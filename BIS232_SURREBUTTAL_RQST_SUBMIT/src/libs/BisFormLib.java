...
public class BisFormLib{
	...
	*/
	public static void fillUpStepFive(HashMap<String, String> row) throws Exception
	{
		...
		for (HashMap.Entry<String, String> entry : row.entrySet()) 
		{
			...
			{
				enterText(replaceGui(guiMap.get("SRE_StepFiveElement"), elementName), entry.getValue());
			}
		}
	...
	*/
	public static boolean ValidateStepFive(HashMap<String, String> row) throws Exception
	{
		...
		holdSeconds(1);
		String errorMsg, elementValue, elementName;
		String htmlToValidate="SRE_StepFiveElementDiv";
		...
		for (HashMap.Entry<String, String> entry : row.entrySet()) 
		{
			...
			if(elementName.equalsIgnoreCase("Scenarios") || elementName.equalsIgnoreCase("Active"))
			continue;
			errorMsg = getElementAttribute(replaceGui(guiMap.get("SRE_StepFiveElementMessage"), elementName), "text");
			switch(elementName)
			{
				case "CompanyName": 
				{
	...
	*/
	public static void searchExclusiveRequestById(String reqId) throws Exception
	{
		...
		holdSeconds(2);
		int currentTimeOut = setBrowserTimeOut(10);
		if(checkElementExists(replaceGui(guiMap.get("tdSearchedId"),reqId)) )
		{
			...
			if(statusReq.equalsIgnoreCase("Pending-Surrebuttal Window Open"))
			{
				...
				clickElementJs(guiMap.get("createObjectionSURRebuttal"));
				holdSeconds(3);
			}
			else
			{
				highlightElement(replaceGui(guiMap.get("tdSearchedStatus"),reqId), "red");
				failTestCase("Search for exclusive request: "+reqId, "The request is at 'Pending-Surrebuttal Window Open'", 
